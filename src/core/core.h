...
bool core::Entity::has_component() noexcept {
    return this->components.find(typeid(t).hash_code()) != this->components.end();
}
...
void core::Entity::remove_component() {
    if(this->has_component<t>()) {
        ...
        throw std::runtime_error("Tried to remove " + std::string(typeid(t).name()) + ", but it doesn't exist!");
    }
}
